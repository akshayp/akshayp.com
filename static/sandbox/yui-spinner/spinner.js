...
YUI.add(
        ...
        const Spinner = Y.Base.create(
            ...
            [],
            {
                ...
                _fill: function(i, length, width, color, lines, radius) {
                    const node = Node.create(div).setStyles({
                        position: 'absolute',
                ...
                _addAnimation: function(alpha, trail, i, lines) {
                    let name = ['opacity', trail, Math.floor(alpha * 100), i, lines].join('-'),
    ...
    '0.0.1',
    { requires: ['node', 'event', 'stylesheet', 'widget', 'base'] }
);
